...
export default function Hero() {
    ...
    return (
        <section id="hero" className="min-h-screen overflow-x-hidden flex flex-col items-center justify-center px-4">
            <div className="flex flex-col md:flex-row items-center justify-start gap-16 w-full max-w-7xl">
                {/* Lado izquierdo: Logo + íconos */}
                <div
                    ref={containerRef}
                    className={`relative ${isMobile ? 'w-[320px] h-[320px]' : 'w-[605px] h-[600px]'}`}
                >
                    <div className={`absolute left-1/2 top-1/2 md:left-1/4 md:top-1/2 -translate-x-1/2 -translate-y-1/2 w-[120px] h-[120px] md:w-[350px] md:h-[350px] `}>
                ...
                <div className={`flex flex-col items-center md:items-start text-center md:text-left gap-4 px-2 transition-opacity duration-700 ${animateOnMount ? 'fade-in-start' : ''}`}>
            ...
            {/* Animaciones CSS en línea (sin cambios) */}
            <style jsx>{`
                @keyframes float {
